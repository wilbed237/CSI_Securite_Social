...
const schema = z.object({
  ...
  type: z.enum(['GENERALIST', 'SPECIALIST']),
...
export function DoctorCreatePage() {
  ...
  const { register, control, handleSubmit, formState: { errors, isSubmitting } } = useForm<FormValues>({ resolver: zodResolver(schema), defaultValues: { type: 'GENERALIST' } });
  const type = useWatch({ control, name: 'type' });
  const onSubmit = async (values: FormValues) => {
    try {
      await authApi.createDoctorAccount({
        username: values.username,
        firstName: values.firstName,
        lastName: values.lastName,
        doctorType: values.type,
        specialty: values.specialty || undefined,
        phoneNumber: values.phoneNumber || undefined,
        email: values.email,
        password: values.password,
      });
      toast.success('Compte médecin créé');
    ...
    <div>
      ...
      <Card>
        <form className="grid gap-4 md:grid-cols-2" onSubmit={handleSubmit(onSubmit)}>
          ...
          <Input label="Mot de passe initial" type="password" required error={errors.password?.message} {...register('password')} />
